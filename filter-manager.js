...
const KM77FilterManager = (function () {
  ...
  function checkScrollPositionForLoadMore() {
    ...
    if (KM77.filterStatusDiv && KM77.filterStatusDiv.style.display === "block") {
      const scrollPosition = window.scrollY + window.innerHeight;
      const documentHeight = document.documentElement.scrollHeight;
      
      // If we're within 500px of the bottom or scrolled to bottom, try to trigger load more
      if (documentHeight - scrollPosition < 500 || (scrollPosition + 5) >= documentHeight) {
        triggerLoadMore();
      }
    }
  }
  ...
  function setupAutoLoadMoreChecker(isActive) {
    ...
    if (isActive) {
      loadMoreCheckInterval = setInterval(() => {
        ...
        const visibleRows = Array.from(KM77.mainTable.querySelectorAll("tbody tr.search"))
        ...
        if (visibleRows.length > 0) {
          ...
          if (rect.bottom <= window.innerHeight + 300) {
            ...
            if (now - lastAttemptTime > 5000) {
              lastAttemptTime = now;
              console.log("KM77 Customizer: Auto-triggering load more from interval checker");
              triggerLoadMore();
            }
          }
        }
  ...
  function triggerLoadMore() {
    ...
    const isLoading = pagedContent.getAttribute("data-paged-content-loading") === "true";
    ...
    if (nextUrl && !isLoading) {
      ...
      const loadMoreButton = document.querySelector(".js-paged-content-load-more");
      if (loadMoreButton) {
      ...
      setTimeout(() => {
        try {
          ...
          console.error("KM77 Customizer: Error injecting load more script:", error);
        }
      }, 300);
    }
  }
  ...
  function checkIfLoadSucceeded(previousNextUrl) {
    ...
    const currentNextUrl = pagedContent.getAttribute("data-paged-content-next-url");
    ...
    GM_xmlhttpRequest({
      ...
      onload: function(response) {
        if (response.status === 200) {
          ...
          // Find new rows
          const newRows = doc.querySelectorAll('tr.search');
          console.log(`KM77 Customizer: Direct fetch found ${newRows.length} rows`);
          ...
          if (newRows.length > 0) {
            ...
            if (tbody) {
              ...
              if (pagedContent && newPagedContent) {
                const newNextUrl = newPagedContent.getAttribute('data-paged-content-next-url');
                if (newNextUrl) {
                  pagedContent.setAttribute('data-paged-content-next-url', newNextUrl);
              ...
              console.log("KM77 Customizer: Successfully added new rows via direct fetch");
              if (KM77.statusDiv) {
                KM77.statusDiv.style.display = "none";
              }
            }
          ...
          console.error(`KM77 Customizer: Direct fetch failed with status ${response.status}`);
          if (KM77.statusDiv) {
  ...
  function setupScrollMonitoring() {
    ...
    document.addEventListener("km77NewRowsAdded", function(event) {
      ...
      setTimeout(() => {
        ...
        if (window.KM77TableManager && typeof KM77TableManager.processExistingRows === 'function') {
    ...
    console.log("KM77 Customizer: Enhanced scroll monitoring for load more initialized");
  }
